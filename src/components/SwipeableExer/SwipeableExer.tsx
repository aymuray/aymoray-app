...
const SwipeableExer = ({ children, dataExercise, dia, getExercices}) => {
  const deleteRecipe = async () => {
    dataExercise.dias = dataExercise.dias.filter(item => item !== dia);
    console.log(dataExercise.dias, dia)
    dataExercise.dias.length === 0
      ? await deleteDoc(doc(db, "Ejercicios", dataExercise.id)).then(() => {})
      : await setDoc(
          doc(db, "Ejercicios", dataExercise.id),
          dataExercise
        ).then(() => {});
    getExercices();
  };
